...
## [현재 상태]
- [Developer] 플레이어 탄환 풀 및 발사 메커니즘 C# 구현 및 테스트 완료 ➔ git_manager에게 커밋/PR 인계
...
## [개발 요소 제안항목]
- [ ] [최적화] 탄환 프리팹 인스턴스화 시 불필요한 GC 방어를 위해 씬 로드 시 고정 수량 풀링(Pre-warm) 방식 적용 제안
- [ ] [확장성] 적(Enemy) 피격 시 사용할 공용 피격 인터페이스 `IDamageable` 도입 제안
