# 유니티 프로젝트 표준 폴더 구조 및 에셋 네이밍 색인 (Folder Structure & Naming Index)

이 문서는 프로젝트의 모든 유니티 폴더 구조, 에셋 배치 위치, 테스트 코드 분류 및 네이밍 컨벤션을 규정하는 중앙 참조 색인 문서입니다.
...
## 4. 에이전트 준수 의무
- **`Developer`**: 신규 스크립트, 프리팹, ScriptableObject, 머티리얼, 테스트 코드 생성 시 본 색인의 폴더 경로와 접두사를 반드시 준수합니다.
- **`Artist`**: AI 에셋 및 스프라이트 생성 시 `Assets/_Imports/` 경로 및 네이밍 접두사를 준수합니다.
- **`QA`**: 검수 시 에셋 및 테스트 코드가 올바른 폴더(`Tests/Editor/`, `Tests/Runtime/`)에 위치하고 명명 컨벤션을 준수했는지 확인합니다.
