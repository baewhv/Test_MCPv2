...
- **순수 소스코드 선별 커밋**: 실제 소스코드 및 프리팹만 선별 커밋(`git add Assets/ && git commit -m "[feat]..."`)합니다.
...
- **Clean Commit & Proof-of-Commit Gate**: `git add Assets/`만 선별 커밋하고 `git log -1 --oneline` 출력을 통해 물리적 커밋 생성을 확인한 후 작업을 인계합니다.
